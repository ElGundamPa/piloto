...
export default function CTADestino({ destinoNombre }: CTADestinoProps) {
  const handleWhatsAppClick = () => {
    const message = `Hola, me interesa el viaje a ${destinoNombre}. ¿Pueden darme más información y cotización?`
      ...
      <div className="container-custom">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-xl mb-10 text-white/90">
            Habla con uno de nuestros asesores y obtén una cotización personalizada para {destinoNombre}
          </p>
          <button
            onClick={handleWhatsAppClick}
            className="btn-whatsapp text-lg px-10 py-5 bg-white text-whatsapp hover:bg-gray-50 
                     shadow-2xl transform hover:scale-105 transition-all duration-300"
            aria-label="Hablar por WhatsApp y cotizar"
          >
            <svg 
              className="w-6 h-6" 
              fill="currentColor" 
